...
| `leave` | H→全C | `{id}` | 切断検知時 |
| `snapshot` | H→C(個別) | `{objs:[{nid, owner, data?}]}` | レイトジョイン用。ホスト所有物は新規Encode、他人の物は最後に受信した `holder.GetData()` |
...
| `pos` | 各自→全員 | `{pos, rot}` | プレイヤー位置(10Hz、移動時のみ)。対象プレイヤーは s から特定 |
...
- **ボイス / アバター / Station同期 / PlayerData・Persistence** 未対応
